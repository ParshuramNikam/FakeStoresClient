...
const Dashboard = () => {
		...
		await fetch('http://localhost:8000/api/auth/protected', {
			...
			.then((result) => {
				if (result.status === 'success') {
					...
					console.warn(">> User Details: ", result.user);
				} else {
	...
	const getCartDetails = async () => {
		console.log(token);
		await fetch(`http://localhost:8000/api/carts/by/cookie?accessToken=` + Cookies.get('access_token'), {
			method: "GET",
			// credentials: "include"
		}).then((res) => res.json())
			.then((result) => {
	...
	const getUserAddedProducts = async () => {
		console.log(token);
		await fetch('http://localhost:8000/api/products/by/cookie?accessToken=' + Cookies.get('access_token'), {
			method: "GET",
			// credentials: "include"
		}).then((res) => res.json())
			.then(async (result) => {
